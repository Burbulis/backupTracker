...
#ifndef _TEMP_CREATOR_
#define _TEMP_CREATOR_
...
namespace TempObjects
{
	...
	struct autoTempCreator
	{
		...
		std::optional<std::array<SQLCMD::types::ul_long,2> > array;
		std::string _fileGuid;
		autoTempCreator():array(std::nullopt)
		{ }

		autoTempCreator(std::string fileGuid):
		array(std::nullopt),
		_fileGuid(fileGuid)
		{ }
		...
		void operator()(sqlType& sqlCache)
		{
			...
			if (!array)
			{
				array = layerHashes(_fileGuid);
			}		
	//		array = // (!array) ? layerHashes(_fileGuid) : std::nullopt;
			if (!complete())
			{
				std::cout << "(1)Fucked table!!!!" << ((array)? "OK" : "Error") << std::endl;
				getc(stdin);
				throw( std::runtime_error("(1)Fucked table!!!!"));
			}
			std::array<SQLCMD::types::ul_long,2> _array = *array;

			SQLCMD::types::ul_long newMaxLayer = array->at(LAST_LAYER_HASH);
			SQLCMD::types::ul_long prevLayer   = array->at(PREV_LAYER_HASH);
		#ifdef LAYERS_HASHES
				_LOG::out_s << "_fileGuid = " 
				<< _fileGuid 
				<< " prevLayer = " 
				<< prevLayer 
				<< " newMaxLayer = " 
				<< newMaxLayer 
				<< " _array[LAST_LAYER_HASH] = " 
				<< array->at(LAST_LAYER_HASH) 
				<< std::endl;
				LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,_LOG::out_s);
		#endif

			createTempTable(sqlCache , _fileGuid , prevLayer , newMaxLayer);
		}
		...
		//	array = (!array) ? layerHashes(_fileGuid) : array;	
			std::array<SQLCMD::types::ul_long,2> _array = *array;
		...
		layerHashes(std::string fileGuid)
		{
			...
			Layers::layersHelper _detectMaximalIdTotal;
			//LayersIO::layersHelper 
			Layers::layersHelper _detectMaximalIdByFileGuid(fileGuid);
			_detectMaximalIdByFileGuid.setLayers();

			SQLCMD::types::ul_long lastLayerIdByFileGuid = _detectMaximalIdByFileGuid.getLastLayerId(); //getLastLayer< SQLCMD::types::ul_long >("layerId");
			SQLCMD::types::ul_long lastLayerIdTotal = _detectMaximalIdTotal.getLastLayerId();
		#ifdef LAYERS_HASHES
				_LOG::out_s << "fileGuid = " << fileGuid << " lastLayerIdByFileGuid = " <<  lastLayerIdByFileGuid  << " lastLayerIdTotal = " << lastLayerIdTotal << std::endl;
				LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,_LOG::out_s);
		#endif
			std::optional<SQLCMD::types::ul_long> 	prevLayerLayerHash = _detectMaximalIdByFileGuid.getHashById(lastLayerIdByFileGuid);
			std::optional<SQLCMD::types::ul_long>   nextLayerLayerHash = 
			(lastLayerIdByFileGuid < lastLayerIdTotal) ? _detectMaximalIdTotal.getHashById(++lastLayerIdByFileGuid) : std::nullopt;
			if ((nextLayerLayerHash) && (prevLayerLayerHash))
			{
			#ifdef LAYERS_HASHES
				_LOG::out_s 
				<< "Layer Not Created = " 
				<< " prevLayerLayerHash =" 
				<< ((prevLayerLayerHash)? *prevLayerLayerHash : 101)  
				<< " nextLayerLayerHash = " 
				<<  ((nextLayerLayerHash)? *nextLayerLayerHash : 101)
				<< std::endl;
				LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,_LOG::out_s);
			#endif
			}
			...
			{
				std::string guid = _detectMaximalIdTotal.createNewLayer();
				nextLayerLayerHash = _detectMaximalIdTotal.getHashById(*_detectMaximalIdTotal.getIdByGuid(guid));
				LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,"NEW_LAYER_CREATED!!!");
		#ifdef LAYERS_HASHES
				_LOG::out_s << "new layerCreated = " 
				<< ((prevLayerLayerHash)? *prevLayerLayerHash : 101)  
				<< ((nextLayerLayerHash)? *nextLayerLayerHash : 101) 
				<< " lastLayerIdByFileGuid = " 
				<<  lastLayerIdByFileGuid  
				<< std::endl;
				LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,_LOG::out_s);
		#endif
	...
	std::string update()
	{
		...
		SQLCMD::Generator <  SQLCMD::selector_type_v, SQLCMD::Update > fUpdate("test_table", SQLCMD::get_ordered(m_update));
		return fUpdate();
	}
	...
	std::string createSubTemp(SQLCMD::types::ul_long LAYER_HASH_ID,SQLCMD::types::ul_long status_main,SQLCMD::types::ul_long status_join)
	{
		...
		dropIfExists("_tmp");
		SQLCMD::Generator <  SQLCMD::selector_type_v, SQLCMD::Create< > > create("_tmp", SQLCMD::type_v(m_create));
	...
	std::string tableCopy(void) const
	{
	...
	*/
	#define CREATE_TEMP_TABLE_LOG_ON
		template
		<
		...
		//#ifdef CREATE_TABLE_FUNCTION_DEBUG
			...
			#define SET_VALDESC_DISTINCT_DEFAULT(x) SQLCMD::valdesc(SQLCMD::selector_cmd_type::NONE, std::vector<std::string>({ x }), SQLCMD::selector_cmd_type::NONE,SQLCMD::valdesc::DISTINCT ,true)
			...
			SQLCMD::multi_selector_type m_select = {
				...
				{ "blockHash", SET_VALDESC_DISTINCT_DEFAULT(joinedTable)},
