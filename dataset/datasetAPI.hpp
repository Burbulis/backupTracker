...
namespace DataSetAPI
{
	...
	#ifdef _GET_SOMETHING_RELATION_NAME_BY_NAME_
	{
			...
			out_s << " relField = "  << relField <<  "  byField = " << byField << " byField_s.size() = " << byField_s.size() << " relField_s.size() = " << relField_s.size()<< std::endl;
			LOGTOFILE(LOGHTML::messageType::WARNING,out_s);
			out_s << "byField_s[0] = " << byField_s[0] << " relField_s[0] " << relField_s[0] << " byValue = " << byValue << std::endl;
			LOGTOFILE(LOGHTML::messageType::WARNING,out_s);
	}
	#endif
	...
	#ifdef _GET_SOMETHING_RELATION_NAME_BY_NAME_
	{
			std::ostringstream out_s; out_s << "n_dist  = " << n_dist << std::endl;
			LOGTOFILE(LOGHTML::messageType::WARNING,out_s);
	}
	#endif
	...
	#ifdef _GET_SOMETHING_RELATION_NAME_BY_NAME_
			{
			std::ostringstream out_s; out_s << "*It_ = " << *It_ << std::endl;
			LOGTOFILE(LOGHTML::messageType::WARNING,out_s);
			}
			END()        
    #endif
