...
namespace TempObjects
{
	...
	//		array = // (!array) ? layerHashes(_fileGuid) : std::nullopt;
		...
		layerHashes(std::string fileGuid)
		{
			...
			SQLCMD::types::ul_long lastLayerIdByFileGuid = _detectMaximalIdByFileGuid.getFirstLayerId(); //getLastLayer< SQLCMD::types::ul_long >("layerId");
	...
	*/
	#define CREATE_TEMP_TABLE_LOG_ON
		...
		//#ifdef CREATE_TABLE_FUNCTION_DEBUG
			...
			SQLCMD::Generator <  SQLCMD::selector_type_v, SQLCMD::Create<false,true> > create("test_table", SQLCMD::type_v(m_create));
			std::string _s = create();
