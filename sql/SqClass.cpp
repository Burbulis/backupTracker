...
//std::list<std::string> sqlQueryCache::cacheSeq;
    ...
    void sqlQueryCache::setQueries(const std::list<std::string> &q)
    {
        cacheSeq = q;
    }

    void sqlQueryCache::insertQueries(const std::list<std::string> &q)
    {
         cacheSeq.insert(cacheSeq.end(),q.begin(),q.end());
    }
    ...
    db(NULL),stmt(NULL)
    {
        ...
        {
             std::ostringstream out_s; out_s <<"fileName = " << fileName << "sqlite error code = " << sqlite3_errcode(db) << " sqlite error : " << sqlite3_errmsg(db) << std::endl;
             THROW(out_s.str());
        }
